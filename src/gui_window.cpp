...
void GuiWindow::renderDrawData(ImDrawData* draw_data) {
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            for (size_t i = 0; i < pcmd->ElemCount; i += 3) {
                ...
                for (int y = min_y; y < max_y; y++) {
                    for (int x = min_x; x < max_x; x++) {
                        ...
                        if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
                            ImU32 col0 = cmd_list->VtxBuffer[idx0].col;
                            ImU32 col1 = cmd_list->VtxBuffer[idx1].col;
                            ImU32 col2 = cmd_list->VtxBuffer[idx2].col;
                            
                            float r = (w0 * ((col0 >> 0) & 0xFF) + w1 * ((col1 >> 0) & 0xFF) + w2 * ((col2 >> 0) & 0xFF));
                            float g = (w0 * ((col0 >> 8) & 0xFF) + w1 * ((col1 >> 8) & 0xFF) + w2 * ((col2 >> 8) & 0xFF));
                            float b = (w0 * ((col0 >> 16) & 0xFF) + w1 * ((col1 >> 16) & 0xFF) + w2 * ((col2 >> 16) & 0xFF));
                            float a = (w0 * ((col0 >> 24) & 0xFF) + w1 * ((col1 >> 24) & 0xFF) + w2 * ((col2 >> 24) & 0xFF));
